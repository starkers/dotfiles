...
function __fundle_load_plugin -a plugin -a path -a fundle_dir -a profile -d "load a plugin"
    if begin
            set -q __fundle_loaded_plugins; and contains $plugin $__fundle_loaded_plugins
        end
        return 0
    end
    ...
    if begin
            test -d $functions_dir; and not contains $functions_dir $fish_function_path
        end
        set fish_function_path $fish_function_path[1] $functions_dir $fish_function_path[2..-1]
    end

    if begin
            test -d $completions_dir; and not contains $completions_dir $fish_complete_path
        end
...
Try reloading your shell if you just edited your configuration."
    ...
    set -l profile 0
    if begin
            contains -- -p $argv; or contains -- --profile $argv; and __fundle_check_date
        end
        set profile 1
    end
...
function __fundle_list -d "list registered plugins"
    if begin
            contains -- -s $argv; or contains -- --short $argv
        end
